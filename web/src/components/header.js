...
const Header = ({ subSiteTitle }) => {
  ...
  const [viewportWidth, setViewportWidth] = useState(Math.max(document.documentElement.clientWidth))
  ...
  return (
    <header className={
      ...
      <div className={navIsShown ? `${styles.header_container} ${styles.open_header}` : styles.header_container}>
        ...
        {viewportWidth < 768 && <div className={styles.logo_wrapper}>
          <Link to='/'><img className={styles.logo} src={logo} alt="Logo PCPR" /></Link>
        </div>}
